...
# use Encoder and Decoder and Supper Computer Value Attention
# (We only use Supper Computer value message to get the attention weight)
...
class SuperCalculateAttentionEncoderDecoder(nn.Module):

    def __init__(self,args):
        super(SuperCalculateAttentionEncoderDecoder, self).__init__()
        self.args = args
        ...
        self.sequenceForDecoderAttention = nn.Sequential(nn.Linear(len(args.supercomput_index), args.seq_len))
    ...
    def forward(self, inputs, predict_M,getAttention=False):
        ...
        for i in range(self.args.pred_len):
            ...
            #todo why pred need to be 2 dimensions
            weights = F.softmax(self.sequenceForDecoderAttention(pred),dim=-1)  # [r_batch_size,args.seq_len]
...
def saendecoder(**kwargs):
    r"""taencoderdecoder, model architecture
    :return:  pure lstm model
    """
    model = SuperCalculateAttentionEncoderDecoder(**kwargs)
    return model
